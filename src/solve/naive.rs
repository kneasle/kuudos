...
impl<'s> Naive<'s> {
    ...
    fn solve_(&self, clues: &Grid, check_multiple_solns: bool) -> Result<(Solution, f32), Error> {
        ...
        // Extract the solution from the partial
        solved_partial.map(|partial| {
            // Extract the solution
            let grid = partial
                .into_solved_digits()
                .expect("Solver returned unsolved grid");
            // Add the difficulty to the return value
            (grid, num_digits_penned as f32)
        })
    }
    ...
    fn recursive_solve(
        ...
        *num_digits_penned += num_naked_singles_solved;
        ...
        for pencilled_digit in partial.pencilled_digit_iter(cell_to_guess).unwrap() {
            ...
            *num_digits_penned += 1;
            ...
            new_partial.pen(&self.table, cell_to_guess, pencilled_digit);
